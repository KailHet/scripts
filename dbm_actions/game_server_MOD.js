module.exports = {
  ...
  <div style="float: left; width: 50%; padding-top: 3px;">
    <span class="dbminputlabel">IP Сервера</span><br>
    <textarea id="serverip" placeholder="mc.example.com" style="width: 90%;"></textarea>
  </div>
  <div style="float: left; width: 50%; padding-left: 10px;  padding-top: 3px;">
    <span class="dbminputlabel">Порт сервера</span><br>
    <textarea id="serverport" rows="2" placeholder="Leave blank for default port" style="width: 90%;"></textarea>
    </div>
  <div style="float: left; width: 97%; padding-top: 15px;">
  <span class="dbminputlabel">Игра</span><br>
...
<div style="float: left; width: 97%; padding-top: 15px;">
  <span class="dbminputlabel">Получаемая информация</span><br>
...
<div>
  ...
  <div id="varNameContainer" style="float: right; width: 50%; padding-right: 3%; padding-top: 15px;">
    <span class="dbminputlabel">Название переменной</span><br>
    <input id="varName" class="round" type="text"><br>
</div>
</div>
  <div style="float: left; width: 88%; padding-top: 15px;">
    <br>
    <p>
      <span><a href="https://www.npmjs.com/package/gamedig#games-with-additional-notes">Список поддерживаемых игр</a></span>
    </p>
  <div>
</div>
<style>
  ...
  async action(cache) {
    ...
    Gamedig.query({
      ...
      .then((state) => {
        let result;
        switch (info) {
          case 0: // Название сервера
            result = state.name;
            break;
          case 1: // Карта
            result = state.map;
            break;
          case 2: // Кол-во игроков
            result = state.players.length;
            break;
          case 3: // Кол-во ботов
            result = state.raw.numbots;
            break;
          case 4: // Макс. кол-во игроков
          ...
          case 6: // Пароль?
            ...
            break;
          case 7: // Список игроков
            result = state.players.map((a) => a.name);
            break;
          case 8: // Режим (проверено на Garry's Mod)
            result = state.game
            break;
          case 9: // Пинг (проверено на Garry's Mod)
            result = state.ping
            break;
          case 10: // Подключение (ip:port)
        ...
        if (result !== undefined) {
          ...
          //   if (state.players[i].raw.score !== undefined) {
          //     let pl_name = state.players[i].name
          //     let pl_score = state.players[i].raw.score
          //     let pl_time = state.players[i].raw.time
          //     res += `${pl_name}: ${pl_score} | ${pl_time.toFixed(0)}\n` 
          //   }
          //   // console.log(pl_score)
          // }
          // console.log(res.replaceAll(`undefined`, ``))
